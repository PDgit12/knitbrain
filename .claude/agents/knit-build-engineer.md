...
<!--
  Vendored by engram from:
    ...
    @6f804f0cfab22fb62668855aa3d62ee3a1453077/categories/06-developer-experience/build-engineer.md
...
**High-fanout files (change carefully):**
- `src/ccr/store.ts`
- `src/tokenizer.ts`
...
- `src/engine/teams.ts`
- `src/engine/meter.ts`
- `src/optimizer/router.ts`
- `src/engine/skills.ts`
- `src/optimizer/types.ts`

**Untested source files (sample):**
- `eslint.config.js`
- `scripts/bench.ts`
...
## Knit MCP tools you can call

You have access to Knit's MCP. Call these when you need depth:
- `knit_query_dependents(file_path)` — what depends on a file
- `knit_get_false_positives()` — full FP list, not just what's above
- `knit_search_learnings(domains)` — search past lessons by tag
- `knit_search_sessions(query)` — has a past session touched this area?
