...
async function fullMcpSession(bin, cwd) {
  ...
  try {
    ...
    ok(names.length === 28, `tools/list advertises exactly 28 tools (got ${names.length})`);
...
console.log(failures === 0
  ? "[audit] VERDICT: PASS — cold-start portable: clone → install → gates → packed install → all 28 tools + proxy + hook + dashboard + hub work."
  : "[audit] VERDICT: FAIL");
