...
async function fullMcpSession(bin, cwd) {
  ...
  try {
    ...
    ok(names.length === 34, `tools/list advertises exactly 34 tools (got ${names.length})`);
...
console.log(failures === 0
  ? "[audit] VERDICT: PASS — cold-start portable: clone → install → gates → packed install → all 34 tools + proxy + hook + dashboard + hub work."
  : "[audit] VERDICT: FAIL");
