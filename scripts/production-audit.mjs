...
async function fullMcpSession(bin, cwd) {
  ...
  try {
    ...
    ok(names.length === 23, `tools/list advertises exactly 23 tools (got ${names.length})`);
...
console.log(failures === 0
  ? "[audit] VERDICT: PASS — cold-start portable: clone → install → gates → packed install → all 23 tools + proxy work."
  : "[audit] VERDICT: FAIL");
