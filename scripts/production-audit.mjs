...
async function fullMcpSession(bin, cwd) {
  ...
  try {
    ...
    ok(names.length === 24, `tools/list advertises exactly 24 tools (got ${names.length})`);
...
console.log(failures === 0
  ? "[audit] VERDICT: PASS — cold-start portable: clone → install → gates → packed install → all 24 tools + proxy work."
  : "[audit] VERDICT: FAIL");
